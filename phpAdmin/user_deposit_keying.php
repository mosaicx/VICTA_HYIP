<?php
	...
	if(isset($_SESSION["ADMIN_ID"])){
		...
		if($AdminUserHandler->getRecordCount() > 0){
			...
			$AdminUserHandler->selectAdminUserByAuid($AdminUID);
		}else{
			session_unset();
	...
	if(isset($_POST["depositSubmit"])){
		...
		$UserHandler ->selectUserByEmail($_POST['depositReference']);
		if($UserHandler->getRecordCount() > 0){
			if($_POST['depositAmt'] > 0){
				$AUID = $UserHandler->getGUID();
				$TXID = bin2hex(openssl_random_pseudo_bytes(10));
				$AmountReceived = $_POST['depositAmt'];
				...
				if($DepositHandler->CreateDeposit($AUID, $AmountReceived, $SenderAddress, $Currency, $ExchangeRate, $LoggedBy)){
					$bankDetailsMsg = 'Deposit success.';						
				}else{
					$bankDetailsMsg = 'Failed to add deposit record.';
				}
			}else{
				$bankDetailsMsg = 'Amount entered must be greater than zero.';
			}
		}else{
			$bankDetailsMsg = 'No user found with this deposit reference.';
		}
	}
?>
<html>
...
<body>
	...
	<div class="w3l-banner">
		...
		<div class="features">
		<div class="container">
			<h3>User Bank Deposit Keying</h3>
						...
						<div role="tabpanel" class="tab-pane fade active in" id="home" aria-labelledby="home-tab">
							<div class="w3agile_tabs">
								<div class="col-md-7 w3agile_tab_left contact-w3-agileits">
									<h4>Add User Deposit</h4>
									<p> This facility lets you manually add deposits to user accounts using bank reference to assign deposits to relevant clients.</p>
									<form enctype="multipart/form-data" name="depositForm" method="post">
										<p><input type="text" name="depositReference" placeholder="Deposit reference (email)*" required=""></p>
										<p><input type="text" name="depositAmt" placeholder="Deposit Amount*" required=""></p>
										<p>							
										Currency 
										...
										<p><input type="text" name="exchange_rate" placeholder="Exchange Rate*" required=""></p>
										<p><input type="Submit" name="depositSubmit" value="Submit"><br/>
